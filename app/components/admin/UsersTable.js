...
export default function UsersTable({ users }) {
  return (
    <div className="overflow-x-auto -mx-5 px-5 md:mx-0 md:px-0">
      <div className="rounded-2xl border border-colordark/6 overflow-hidden mb-12 min-w-[800px]">
        <table className="w-full border-collapse">
          <thead>
            <tr className="border-b border-colordark/6">
              {["User", "Plan", "Status", "Cases", "Joined", ""].map((h, i) => (
                <th
                  key={i}
          ...
          <tbody className="divide-y divide-colordark/4">
            {users.map((user) => (
              <tr key={user.id} className="hover:bg-colordark/1.5 transition-colors cursor-pointer group">
                <td className="px-6 py-4">
                  <p className="text-[0.875rem] font-semibold text-colordark group-hover:text-blue-from transition-colors">
                    {user.name}
                  </p>
                  <p className="text-[0.75rem] text-colordark/40">{user.email}</p>
                </td>
                <td className="px-6 py-4">
                  <span className={`inline-flex px-2.5 py-1 rounded-full text-[0.6875rem] font-bold uppercase tracking-wide ${planBadge(user.plan)}`}>
                    {user.plan}
                  </span>
                </td>
                <td className="px-6 py-4">
                  <div className="flex items-center gap-1.5">
                    {user.status === "Active" ? (
                      ...
                      <>
                        ...
                        <span className="text-[0.8125rem] text-red-500 font-medium">Inactive</span>
                      </>
                    )}
                  </div>
                </td>
                <td className="px-6 py-4">
                  <span className="text-[0.875rem] text-colordark font-medium">{user.cases}</span>
                </td>
                <td className="px-6 py-4">
                  <span className="text-[0.8125rem] text-colordark/45">{user.joined}</span>
                </td>
