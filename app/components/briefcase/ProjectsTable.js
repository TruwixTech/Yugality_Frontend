...
import { Star, Download, Trash2, Folder } from "lucide-react";

export default function ProjectsTable({ projects, filtered, onRowClick, onStar, onDelete }) {
  return (
    <>
      {/* ─── Desktop Table (hidden on mobile) ─── */}
      <div className="hidden md:block rounded-2xl border border-colordark/6 overflow-x-auto">
        <table className="w-full border-collapse min-w-[700px]">
          <thead>
            <tr className="border-b border-colordark/[0.06]">
              <th className="text-left px-6 py-3 text-[0.8125rem] font-semibold text-colordark/50 w-[50%]">Name</th>
              <th className="text-left px-4 py-3 text-[0.8125rem] font-semibold text-colordark/50">Last Updated</th>
              <th className="text-left px-4 py-3 text-[0.8125rem] font-semibold text-colordark/50">Owner</th>
          ...
          <tbody className="divide-y divide-colordark/[0.04]">
            {filtered.map((project) => (
              ...
              >
                ...
                <td className="px-4 py-3.5 text-[0.875rem] text-colordark/55">{project.lastUpdated}</td>
                <td className="px-4 py-3.5 text-[0.875rem] text-colordark/55">{project.owner}</td>
                <td className="px-6 py-3.5">
                  <div className="flex items-center justify-end gap-1">
                    <button onClick={(e) => onStar(e, project.id)} className={`p-1.5 rounded-lg transition-all ${project.starred ? "text-amber-500" : "text-colordark/30 hover:text-amber-500"}`} title="Star">
                      <Star size={15} strokeWidth={2} className={project.starred ? "fill-amber-500" : ""} />
                    </button>
                    ...
                    <button onClick={(e) => onDelete(e, project.id)} className="p-1.5 text-colordark/30 hover:text-red-500 rounded-lg transition-all" title="Delete">
        ...
        <div className="px-6 py-3 border-t border-colordark/6 flex items-center gap-6">
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-1">
                  <span className="text-[0.75rem] text-colordark/45">{project.lastUpdated}</span>
                  <span className="text-colordark/20">·</span>
                  <span className="text-[0.75rem] text-colordark/45">{project.fileCount} files</span>
                </div>
              </div>
              <div className="flex items-center gap-0.5 shrink-0">
                <button onClick={(e) => onStar(e, project.id)} className={`p-1.5 rounded-lg transition-all ${project.starred ? "text-amber-500" : "text-colordark/30"}`}>
                  <Star size={15} strokeWidth={2} className={project.starred ? "fill-amber-500" : ""} />
                </button>
                <button onClick={(e) => onDelete(e, project.id)} className="p-1.5 text-colordark/30 rounded-lg transition-all">
                  <Trash2 size={15} strokeWidth={2} />
                </button>
              </div>
            </div>
